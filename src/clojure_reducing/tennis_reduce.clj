...
(defn serena-williams-win-loss-streaks [matches]
  (:matches 
   (reduce (fn [{:keys [current-wins current-losses] :as acc}
             ...
             (let [this-match (assoc match 
                                     :current-streak 
                                     (streak-string current-wins current-losses))
                   serena-vistory? (= winner-name "Williams S.")]
               (-> acc
                   (update :matches #(conj % this-match))
                   (assoc :current-wins (if serena-vistory? (inc current-wins) 0))
                   (assoc :current-losses (if serena-vistory? 0 (inc current-losses))))))
          {:matches []
           :current-wins 0
           :current-losses 0}
          matches)))